...
const numbersOnly = (items) => {
  let numbersOnly = items.filter((item) => {
...
const isAllSame = (items) => {
  let same = items.every((item, i, arr) => {
    return arr[0] === item;
  });
  return same;
};
...
const stringsAndCaps = (items) => {
  let newString = items.filter((item) => {
