...
const numbersOnly = (items) => {
  return items.filter(item => typeof(item) === 'number' ? true : false);
};
...
const targetCount = (nums, target) => {
  ...
  nums.forEach(num => {
    num === target ? targetCounter++ : false;
  })
...
const tripledAndLessThan20 = (nums) => {
  return nums.map(num => num * 3).every(num => num < 20);
};
...
const arePositiveEvenAndNonZeroEnding = (nums) => {
  return nums.every(num => ((num > 0 && num%2 === 0) && num%10 !== 0)
   ? true : false);
};
