...
const zeroCount = (nums) => {
  let zeroCount = 0
  nums.forEach(num => zeroCount += num === 0 ? 1 : 0)
  return zeroCount
};
...
const targetCount = (nums, target) => {
  return nums.reduce((accum, num) => num === target ? accum + 1 : accum, 0)
};
...
const counterObject = (items) => {
  let obj = {}
  items.forEach(item => obj[item] = (obj[item] || 0) + 1)
  return obj
};
...
const arePositiveEvenAndNonZeroEnding = (nums) => {
  return nums.every(num => num > 0 && num % 10 !== 0)
};
...
const stringsAndCaps = (items) => {
  return items.filter(item => typeof item === 'string').map(str => str.toUpperCase())
};
