...
const numbersOnly = (items) => {
  let numArray = items.filter((items) => {
...
const isAllSame = (items) => items.every((item) => item === items[0]);
...
const isAllOdd = (nums) => nums.every((num) => num % 2 === 1);
...
const doubled = (nums) => nums.map((num) => num * 2);
//   let  elDoubled = nums.map((num) => {
//      return  num * 2
//   })
// return elDoubled
// };
...
const stringsAndCaps = items =>  items.filter(item => typeof item === "string").map(item => item.toUpperCase())
