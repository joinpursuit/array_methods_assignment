...
const stringsOnly = (items) => {
  let emptyStringArr = items.map(item => {
...
const numberTimesIdx = (nums) => {
  return nums.map(num => num * nums.indexOf(num))
};
...
const arePositiveEvenAndNonZeroEnding = (nums) => {
  return nums.every(num => num % 2 === 0 && Math.sign(num) === 1 && num.toString().slice(-1) !== '0')
};
...
const stringsAndCaps = (items) => {
  let stringWords = items.filter(item => {
    if(typeof item === 'string'){
      return item;
    }
  });

  let uppercaseWords = stringWords.map(word => word.toUpperCase())
