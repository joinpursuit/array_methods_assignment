...
// b6 //
console.log("6. Write a `myJoin` function.");
let input_b06 = ['ichi', 'ni', 'san', 'shi'];
console.log(`Input: ${input_b06}. Join with '-'`);

Object.defineProperty(Array.prototype, 'myJoin', {
    value: function(pmSeparator) {
      let newStr = this[0];
      pmSeparator === undefined
        ? pmSeparator = ','
        : false;
      for (let i = 1; i < this.length; i++) {
        newStr += (pmSeparator + this[i]);
      }
      return newStr;
    }
} );
let resultb06 = input_b06.myJoin('-');
print(resultb06, 1);
//


// b7 //
console.log("7. Write a `mySlice` function.");
let input_b07 = ['a','b','l','i','o','n','c'];
console.log(`Input: ['a','b','l','i','o','n','c']`);

Object.defineProperty(Array.prototype, 'mySlice', {
    value: function(pmBegin, pmEnd) {
      let outputArray = [];

      // formalize pmBegin
      if (pmBegin === undefined) {
        pmBegin = 0;
      }
      if (pmBegin > this.length) {
        return outputArray;
      }
      if (pmBegin < 0) {
        pmBegin = this.length + pmBegin;
      }

      // formalize pmEnd
      if (pmEnd === undefined || pmEnd > this.length) {
        pmEnd = this.length;
      }
      if (pmEnd < 0) {
        pmEnd = this.length + pmEnd;
      }

      for (let i = pmBegin; i < pmEnd; i++) {
        outputArray.push(this[i]);
      }
      return outputArray;
    }
} );
let resultb07 = input_b07.mySlice(-5,-1);
print(resultb07, 1);
//


// b8 //
console.log("8. Write a `myTranspose`. This function should transpose a matrix.");
let input_b08 = [
  [1, 2], [3, 4], [5, 6]
];
...
Object.defineProperty(Array.prototype, 'myTranspose', {
    value: function() {
      let subOutput_Lefts = [];
      let subOutput_Rights = [];
      let outputArray = [subOutput_Lefts, subOutput_Rights];
      for (let i of this) {
        subOutput_Lefts.push(i[0]);
        subOutput_Rights.push(i[1]);
      }
      return outputArray;
    }
} );
let resultb08 = input_b08.myTranspose();
print(resultb08, 1);
//

/* UNFINISHED ======
// b9 //
console.log("9. Write a `myFlatten`. This should take a multi-dimensional array and return it as one array.");
let input_b09 = [[[-3, -2], -1, 0], 1, 2, [3, 4, 5, [6, 7, 8]]];
console.log('[[[-3, -2], -1, 0], 1, 2, [3, 4, 5, [6, 7, 8]]]');

Object.defineProperty(Array.prototype, 'myFlatten', {
    value: function() {
      let isStillArray;
      while (isStillArray !== false) {
...
let resultb09 = input_b09.myFlatten();
print(resultb09, 1);
//
*/
